...
read -p "Starting build of OpenSSL, Press any key to continue"

## Build OpenSSL static library (libssl.a & libcrypto.a)
$BASE_PATH/jni/compile-openssl.sh
checkExitCode $?

read -p "Starting build of zlib, Press any key to continue"
...
read -p "Starting build of cURL, Press any key to continue"
...
for abi in ${APP_ABI[*]}; do
	...
	x86_64)
		# http://gcc.gnu.org/onlinedocs/gcc/x86-Options.html
		compile $abi "$NDK_ROOT/platforms/android-23/arch-x86_64" "$NDK_ROOT/toolchains/x86-4.9/prebuilt/$host-x86_64/bin" "x84_64-linux-android" "-march=x86_64"
		;;
	arm64-v8a)
